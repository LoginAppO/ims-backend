...
public class FilterUsersQueryHandler : IRequestHandler<FilterUsersQuery, ApplicationResponse<PaginationInfo<ListUsersResponse>>>
{
    private readonly IUserRepository _userRepository;
    private readonly IPaginationRepository<ListUsersResponse> _paginationRepository;

    public FilterUsersQueryHandler(IUserRepository userRepository, IPaginationRepository<ListUsersResponse> paginationRepository)
    {
    ...
    public async Task<ApplicationResponse<PaginationInfo<ListUsersResponse>>> Handle(
        FilterUsersQuery request,
        CancellationToken cancellationToken)
    {
        var usersQuery = _userRepository.Filter(request.SearchQuery ?? string.Empty);
